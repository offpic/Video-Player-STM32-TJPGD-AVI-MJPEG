...
)
{
	...
	for (blk = 0; blk < nby + nbc; blk++) {
		...
		if (b < 0) return JDR_INP;				/* Err: invalid code or input */
		d = jd->dcv[cmp];						/* DC value of previous block */
		if (b) {								/* If there is any difference from previous block */
			e = bitext(jd, b);					/* Extract data bits */
			if (e < 0) return JDR_INP;			/* Err: input */
...
)
{
	...
	LCD_Window(x+rx+7,y+ry+15,16,16);/* Rectangular area in the frame buffer */
	LCD_Cursor(x+8,y+15);
	LCD_REG=0x22;
	for (iy = 0; iy < my; iy++) {
		pc = jd->mcubuf;
		...
		for (ix = 0; ix < mx; ix++) {
			...
			temp=(BYTECLIP(yy+((INT)(1.402*CVACC)*cr)/CVACC)&0xF8)<<8;
			temp|=(BYTECLIP(yy-((INT)(0.344*CVACC)*cb+(INT)(0.714*CVACC)*cr)/CVACC)&0xFC)<<3;
			temp|=BYTECLIP(yy+((INT)(1.772*CVACC)*cb)/CVACC)>>3;
			LCD_RAM=temp; 
		}
	}
	return 	JDR_OK;
...
u8  Work_buf[sz_wbuf]={0x00};
JRESULT jd_prepare (
	JDEC* jd,			/* Blank decompressor object */
	BYTE *fbuf			/* JPEG strem input buffer */
)
{
	BYTE b;
	WORD marker;
	...
	if (LDB_WORD(jd->dptr) != 0xFFD8)return JDR_FMT1;/* Err: SOI is not detected */ 
	jd->dptr+=2;	

	for (;;) {
		/* Get a JPEG marker */	
		marker = LDB_WORD(jd->dptr);/* Marker */	
		len = LDB_WORD(jd->dptr+2);/* Length field */
		jd->dptr+=4;
		...
		case 0xC0:	/* SOF0 (baseline JPEG) */
			/* Load segment data */			
			jd->height = LDB_WORD(jd->dptr+1);/* Image width in unit of pixel */
			jd->width = LDB_WORD(jd->dptr+3);/* Image height in unit of pixel */

			if (jd->dptr[5] != 3) return JDR_FMT3;/* Err: Supports only Y/Cb/Cr format */
			/* Check three image components */
			for (i = 0; i < 3; i++) {								
				b = jd->dptr[7 + 3 * i];/* Get sampling factor */	
				if (!i) {	/* Y component */
				...
				b = jd->dptr[8 + 3 * i];								/* Get dequantizer table ID for this component */
		...
		case 0xC4:	/* DHT */
			...
			rc = create_huffman_tbl(jd, jd->dptr, len);
			if (rc) return rc;
			jd->dptr+=len;
			break;
		...
		case 0xDB:	/* DQT */
			...
			rc = create_qt_tbl(jd, jd->dptr, len);
			if (rc) return rc;
			jd->dptr+=len;
			break;
		...
		case 0xDA:	/* SOS */
			...
			if (jd->dptr[0] != 3) return JDR_FMT3;/* Err: Supports only three color components format */
			/* Check if all tables corresponding to each components have been loaded */
			for (i = 0; i < 3; i++) {	
				b = jd->dptr[2 + 2 * i];/* Get huffman table ID */
